...
export const Cart = () => {
  const { cart, order, updateCartItemQuantity, removeFromCart, addToCart, isLoading, refreshOrder } = useCart()
  ...
  const handleQuantityChange = (item: CartItem, newQuantity: number) => {
    console.log("🔄 Cart.tsx - handleQuantityChange:", { itemId: item.id, currentQuantity: item.quantity, newQuantity })
      ...
      <main className="container mx-auto py-12 px-4 md:px-6">
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">
              ...
              {order && <span className="ml-2 text-xs text-amber-600">(Commande #{order.code})</span>}
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            <ShippingProgress />
            <CartItemsList items={cart} removingItems={removingItems} onQuantityChange={handleQuantityChange} />
            <UpsellSection products={upsellProducts} onAddProduct={handleAddUpsellProduct} />
          </div>
