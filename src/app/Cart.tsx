...
import { Trash2, ShoppingBag, ArrowRight, Minus, Plus, Gift, Truck, Sparkles, Star } from "lucide-react"
...
export const Cart = () => {
  ...
  const subtotal = cart.reduce((total, item) => total + item.variantList.items[0].priceWithTax * item.quantity, 0)
  ...
  // Fonction pour récupérer les produits de manière synchrone
  const loadProducts = () => {
    fetchAllProductsFromApi()
        ...
        .catch((error) => {
          ...
          setIsProductsLoaded(true) // Même en cas d'erreur, on considère le chargement terminé
        })
  }
  ...
  const getFeaturedProducts = (products: Product[]) => {
    return products.filter((product) => product.facetValues.some((facet) => facet.name === "top"))
  }
  ...
  const handleQuantityChange = (item: CartItem, newQuantity: number) => {
    if (newQuantity >= 1 && newQuantity <= item.variantList.items[0].actualStockLevel) {
  ...
  // Affichage de chargement pendant la récupération des produits
  if (!isProductsLoaded) {
    return (
        ...
        <div className="min-h-screen bg-gradient-to-br from-amber-50 via-orange-50 to-yellow-50">
          <div className="container mx-auto py-16 px-4 md:px-6">
            ...
            >
              ...
              <h1 className="text-3xl md:text-4xl font-bold mb-4 font-playfair">Votre panier est vide</h1>
              <p className="text-muted-foreground text-lg mb-8 leading-relaxed">
                Découvrez notre collection exclusive d'articles de luxe pour vos compagnons bien-aimés. Chaque produit est
                soigneusement sélectionné pour offrir le meilleur à votre animal.
              </p>
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center mb-12">
                ...
                <Button asChild variant="outline" size="lg" className="border-amber-200 text-amber-700 hover:bg-amber-50">
            ...
            >
              <div className="text-center mb-12">
                ...
                <p className="text-muted-foreground max-w-2xl mx-auto">
                  Découvrez les produits les plus appréciés par nos clients pour leurs compagnons
                </p>
              </div>
            ...
            >
              <div className="text-center mb-12">
                <h2 className="text-2xl md:text-3xl font-bold mb-4 font-playfair">Explorez par catégorie</h2>
            ...
            >
              <div className="max-w-2xl mx-auto">
                ...
                <h2 className="text-2xl md:text-3xl font-bold mb-4 font-playfair">Offre de bienvenue</h2>
                ...
                <div className="flex flex-col sm:flex-row gap-4 max-w-md mx-auto">
                  ...
                  />
                  <Button className="bg-white text-amber-800 hover:bg-white/90 px-6 py-3 font-medium">S'inscrire</Button>
                </div>
                <p className="text-white/70 text-sm mt-4">Pas de spam, uniquement nos meilleures offres et nouveautés</p>
              </div>
            </motion.section>
          </div>
        </div>
            ...
            <div className="bg-gradient-to-r from-amber-50 to-orange-50 rounded-lg p-6 border border-amber-100">
              <div className="flex items-center justify-between mb-3">
                ...
                {subtotal >= freeShippingThreshold ? (
                    ...
                    <span className="text-sm text-muted-foreground">Plus que {remainingForFreeShipping.toFixed(2)} €</span>
                )}
              </div>
              ...
              {subtotal >= freeShippingThreshold ? (
                  ...
                  <p className="text-sm text-muted-foreground">
                    Ajoutez {remainingForFreeShipping.toFixed(2)} € pour bénéficier de la livraison gratuite
                  </p>
              )}
            </div>
                    ...
                    >
                      <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
                        ...
                        <div className="col-span-1 md:col-span-4">
                          <h3 className="font-medium hover:text-amber-700 transition-colors">{item.name}</h3>
                          <p className="text-sm text-muted-foreground mt-1">
                            {item.facetValues.map(
                                (facetValue, index) =>
                                    facetValue.name &&
                                    facetValue.name !== "top" &&
                                    !facetValue.name.startsWith("carac_") && <span key={index}>{facetValue.name}</span>,
                            )}
                          </p>
                        ...
                        <div className="col-span-1 md:col-span-2 text-left md:text-center">
                          <div className="md:hidden text-sm text-muted-foreground mb-1">Prix</div>
                          <span className="font-medium">{formatCurrency(item.variantList.items[0].priceWithTax, "€")}</span>
                        </div>

                        <div className="col-span-1 md:col-span-2 text-left md:text-center">
                          <div className="md:hidden text-sm text-muted-foreground mb-1">Quantité</div>
                          <div className="flex items-center border rounded-md w-32 mx-auto bg-white">
                            ...
                            <Button
                                ...
                                onClick={() => handleQuantityChange(item, item.quantity + 1)}
                                disabled={item.quantity >= item.variantList.items[0].actualStockLevel}
                            >
                        ...
                        {formatCurrency(item.variantList.items[0].priceWithTax * item.quantity, "€")}
                      </span>
                        </div>
                      </div>
