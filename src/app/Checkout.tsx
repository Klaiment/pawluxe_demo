...
export const Checkout = () => {
    ...
    const subtotal = cart.reduce((total, item) => total + item.variantList.items[0].priceWithTax/100 * item.quantity, 0)
    const shippingCost = checkoutData.shippingMethod === "express" ? 9.99 : subtotal >= 50 ? 0 : 4.99
    ...
    const renderStepContent = () => {
        switch (currentStep) {
            case 1:
                return <CustomerInfoStep data={checkoutData} onUpdate={updateCheckoutData} onNext={nextStep} />
            case 2:
                return (
                    <ShippingStep
                        data={checkoutData}
            ...
            case 3:
                return <PaymentStep data={checkoutData} onUpdate={updateCheckoutData} onNext={nextStep} onPrev={prevStep} />
            case 4:
                return (
                    <OrderReviewStep
                        data={checkoutData}
                ...
                <div className="container mx-auto px-4 md:px-6 py-4">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-4">
                            <Button variant="ghost" onClick={() => router("/cart")} className="hover:bg-amber-50">
                                <ArrowLeft className="mr-2 h-4 w-4" />
                                Retour
                            </Button>
                            <div>
                                <h1 className="text-xl font-bold font-playfair">Finaliser votre commande</h1>
            ...
            <div className="container mx-auto px-4 md:px-6 py-8">
                <div className="max-w-6xl mx-auto">
                    ...
                    <div className="mb-12">
                      ...
                      `}
                                            >
                                                {isCompleted ? <Check className="h-6 w-6" /> : <span>{step.icon}</span>}
                                            ...
                                            <div className="text-center max-w-24">
                                                <div
                                                    className={`text-sm font-semibold transition-colors duration-300 ${
                                                        isActive ? "text-amber-700" : isCompleted ? "text-green-700" : "text-gray-500"
                                                    }`}
                                                >
                                                    {step.title}
                                                </div>
                                                <div className="text-xs text-muted-foreground hidden sm:block mt-1">{step.subtitle}</div>
                                            </div>
                                        </div>
                                    )
                                })}
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                                ...
                                <CardContent className="p-8">
                                    ...
                                    <div className="mb-8">
                                        <div className="flex items-center space-x-3 mb-2">
                                            ...
                                            <h2 className="text-2xl font-bold font-playfair">{steps[currentStep - 1].title}</h2>
                                        </div>
                                        <p className="text-muted-foreground">{steps[currentStep - 1].subtitle}</p>
                                    </div>

                                    {/* Step Content */}
                                    <div className="transition-all duration-500 ease-in-out">{renderStepContent()}</div>
                                </CardContent>
                            </Card>
                        </div>
                        ...
                        <div className="lg:col-span-1">
                            <div className="sticky top-32">
                                <Card className="border-0 shadow-xl bg-white/90 backdrop-blur-sm overflow-hidden">
                                    ...
                                    <div className="bg-gradient-to-r from-amber-600 to-orange-600 p-6 text-white">
                                        ...
                                        <p className="text-amber-100 text-sm mt-1">Commande sécurisée</p>
                                    </div>
                                            ...
                                            {cart.map((item) => (
                                                <div key={item.id} className="flex items-center space-x-3 p-3 bg-amber-50 rounded-lg">
                                                    ...
                                                    <p className="text-sm font-semibold text-amber-800">
                                                        {(item.variantList.items[0].priceWithTax/100 * item.quantity).toFixed(2)} €
                                                    </p>
                                                </div>
                                            ))}
                                        </div>
                          ...
                          {shippingCost === 0 ? "Gratuite" : `${shippingCost.toFixed(2)} €`}
                                        ...
                                        {/* Payment Icons */}
                                        <div className="mt-6 pt-6 border-t border-amber-100">
                                            <p className="text-xs text-muted-foreground mb-3 text-center">Moyens de paiement acceptés</p>
