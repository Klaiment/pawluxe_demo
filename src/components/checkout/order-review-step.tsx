  ...
  return (
    <div className="space-y-6">
        ...
        <div className="space-y-4">
          <div className="flex items-start space-x-3 p-4 bg-amber-50 rounded-lg">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">{data.customerInfo.email}</p>
              <p className="text-sm text-muted-foreground">{data.customerInfo.phone}</p>
            </div>
          </div>
          ...
          <div className="flex items-start space-x-3 p-4 bg-amber-50 rounded-lg">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">{data.billingAddress.street}</p>
              <p className="text-sm text-muted-foreground">
                {data.billingAddress.postalCode} {data.billingAddress.city}
              </p>
              <p className="text-sm text-muted-foreground">{data.billingAddress.country}</p>
            </div>
          </div>
          ...
          <div className="flex items-start space-x-3 p-4 bg-amber-50 rounded-lg">
            ...
            <div>
              ...
              {data.sameAsBilling ? ( // Correction ici: sameAsBilling au lieu de sameAsBinding
                <p className="text-sm text-muted-foreground">Identique à l'adresse de facturation</p>
              ) : (
                <>
                  <p className="text-sm text-muted-foreground">{data.shippingAddress.street}</p>
                  <p className="text-sm text-muted-foreground">
              ...
              <p className="text-sm text-amber-700 font-medium mt-1">{getShippingMethodLabel(data.shippingMethod)}</p>
            </div>
          </div>
          ...
          <div className="flex items-start space-x-3 p-4 bg-amber-50 rounded-lg">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">{getPaymentMethodLabel(data.paymentMethod)}</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-3">
          {cart.map((item) => (
            <div key={item.id} className="flex items-center space-x-3 p-3 border rounded-lg">
              <img src={item.featuredAsset.preview || "/placeholder.svg"} alt={item.name} className="w-12 h-12 object-cover rounded" />
              <div className="flex-1">
                <p className="font-medium">{item.name}</p>
                <p className="text-sm text-muted-foreground">Quantité: {item.quantity}</p>
              </div>
              <p className="font-medium">{(item.variantList.items[0].priceWithTax/100 * item.quantity).toFixed(2)} €</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="flex justify-between">
        ...
        <Button onClick={onSubmit} disabled={isProcessing} className="bg-amber-700 hover:bg-amber-800" size="lg">
          {isProcessing ? (
