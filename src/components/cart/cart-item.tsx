...
export const CartItemComponent = ({ item, isRemoving, onQuantityChange }: CartItemProps) => {
  console.log("CartItem component - item:", item)

  return (
      <div className={`mb-6 transition-all duration-300 ${isRemoving ? "opacity-50 scale-95" : ""}`}>
        <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
          ...
          <div className="col-span-1 md:col-span-4">
            <h3 className="font-medium hover:text-amber-700 transition-colors">{item.productVariant.product.name}</h3>
            <p className="text-sm text-muted-foreground mt-1">{item.productVariant.name}</p>
          ...
          <div className="col-span-1 md:col-span-2 text-left md:text-center">
            ...
            <span className="font-medium">{formatCurrency(item.productVariant.priceWithTax, "€")}</span>
          </div>
          ...
          <div className="col-span-1 md:col-span-2 text-left md:text-center">
            ...
            <span className="font-semibold text-amber-800">{formatCurrency(item.linePriceWithTax, "€")}</span>
          </div>
        </div>
      </div>
  )
